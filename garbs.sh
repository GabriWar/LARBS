...
[ -z "$dotfilesrepo" ] && dotfilesrepo="https://github.com/GabriWar/voidrice.git"
[ -z "$progsfile" ] && progsfile="https://raw.githubusercontent.com/GabriWar/LARBS/master/progs.csv"
...
welcomemsg() { \
	dialog --title "Welcome!" --msgbox "Welcome Gabriwar's Auto-Rice Bootstrapping Script!\\n\\nThis script will automatically install a fully-featured Linux desktop, which I use as my main machine.\\n\\n-Gabriel" 10 60
...
manualinstall() { # Installs $1 manually. Used only for AUR helper here.
	...
	sudo -u "$name" git clone --depth 1 "https://aur.archlinux.org/yay.git" "$repodir/$1" >/dev/null 2>&1 ||
...
gitmakeinstall() {
	...
	dialog --title "GARBS Installation" --infobox "Installing \`$progname\` ($n of $total) via \`git\` and \`make\`. $(basename "$1") $2" 5 70
...
systembeepoff() { dialog --infobox "Getting rid of that dumb error beep sound..." 10 50
	rmmod pcspkr
...
dialog --title "GARBS Installation" --infobox "Finally, installing \`libxft-bgra\` to enable color emoji in suckless software without crashes." 5 70
...
newperms "%wheel ALL=(ALL) ALL #GARBS
...
chmod +x /home/$name/.local/bin/statusbar/*
#setting br layout
